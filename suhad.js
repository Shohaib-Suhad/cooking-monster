const getMealInput = () => {
  const mealInput = document.getElementById("input-s").value;
  const ingredients = document.getElementById("s-ingredients");
  ...
  fetch(url)
    .then((res) => res.json())
    .then((data) => {
      const sListDiv = document.getElementById("s-list");
      let ssInput = data.s;
      let mealList = "";
      mealsInput.forEach((element) => {
        mealList += ` 
          <div onClick="getIngredients('${element.strMeal}')" class="s-card">
    ...
    .catch(function () {
      const mealError = document.getElementById("s-list");
      let mealErrorString = "";
...
const ingredientsOfMeal = (mealsIngredients) => {
  const ingredientsDiv = document.getElementById("s-ingredients");
  ingredientsDiv.style.display = "block";
  ingredientsDiv.innerHTML = `
  <div class="s-ingredients">
